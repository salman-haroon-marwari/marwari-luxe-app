...
tsxFiles.forEach(filePath => {
  try {
    ...
    // Check if file contains www image URLs
    if (imageUrlRegex.test(content)) {
      // Reset regex state
      imageUrlRegex.lastIndex = 0;
      
      // Replace www with non-www
      const updatedContent = content.replace(imageUrlRegex, 'https://marwariluxe.com/og-image.jpg');
...
console.log(`Updated ${updatedFiles} files with non-www image URLs`);