...
sitemapContent = sitemapContent.replace(/\s*<url>[\s\S]*?<loc>https:\/\/www\.marwariluxe\.com\/products\/[^<]+<\/loc>[\s\S]*?<\/url>/g, '');

// Remove all individual blog URLs
sitemapContent = sitemapContent.replace(/\s*<url>[\s\S]*?<loc>https:\/\/www\.marwariluxe\.com\/blogs\/[^<]+<\/loc>[\s\S]*?<\/url>/g, '');
