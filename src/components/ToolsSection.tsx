...
const ToolsSection = () => {
  ...
  const beautyTools = tools.filter(tool => tool.category === 'Beauty');
  
  // Take 2 tools from each category
  const selectedHealthTools = healthTools.slice(0, 2);
  const selectedBeautyTools = beautyTools.slice(0, 2);
  
  // Combine and shuffle the tools
  const selectedTools = [...selectedHealthTools, ...selectedBeautyTools];

  return (
    <section className="py-20 px-4 bg-gray-50">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-16 animate-fade-in">
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">Interactive Tools</h2>
          <p className="text-xl text-foreground/70 max-w-2xl mx-auto">
            Enhance your wellness and beauty journey with our specialized tools
          </p>
        </div>
