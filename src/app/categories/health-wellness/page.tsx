...
export default function HealthWellnessCategory() {
  // Filter custom health blogs (custom-health-001 to custom-health-006)
  const blogs = blogPosts.filter(blog => 
    blog.id.startsWith('custom-health-00') && 
    parseInt(blog.id.replace('custom-health-00', ''), 10) >= 1 && 
    parseInt(blog.id.replace('custom-health-00', ''), 10) <= 6
  );
  
  // Filter health products (first 4 health products)
  const healthProducts = allProducts
    .filter(product => product.id.startsWith('health-'))
    .slice(0, 4);
      ...
      <main className="flex-grow pt-32 pb-16 px-4">
        <div className="container mx-auto max-w-6xl">
          ...
          <section id="products" className="mb-16">
            <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-10 gap-4">
              <div>
                <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-2">Premium Wellness Formulations</h2>
                <p className="text-foreground/70 max-w-2xl">Scientifically-engineered supplements for optimal bioavailability</p>
              </div>
              <Link href="/products" className="px-6 py-3 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-medium rounded-lg hover:from-emerald-600 hover:to-green-700 transition-all duration-300 shadow-md hover:shadow-lg">
                Browse All Products
              </Link>
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {healthProducts.map((product: any) => (
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto">
                      <a 
                        ...
                        className="w-full py-2 bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-lg hover:from-emerald-600 hover:to-green-700 transition-colors text-sm font-medium text-center block"
                      >
                        Buy Now
                      </a>
                    </div>
